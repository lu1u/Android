...
public class PhotosReader extends SavedObjectReader
{
	...
	public PhotosReader( Context c, long depuis)
	{
		super( c, depuis, c.getContentResolver().query(Images.Media.EXTERNAL_CONTENT_URI, 
	            COLONNES, 
	            MediaColumns.DATE_MODIFIED + " >= " + depuis,  //$NON-NLS-1$
	            null, 
	            MediaColumns.DATE_MODIFIED + " ASC")); //$NON-NLS-1$

		_colonneAbsolutePath = _cursor.getColumnIndexOrThrow(MediaColumns.DATA);
		_colonneDisplayName = _cursor.getColumnIndexOrThrow(MediaColumns.DISPLAY_NAME);
		_colonneDescription = _cursor.getColumnIndexOrThrow(VideoColumns.DESCRIPTION);
		_colonneDateModified = _cursor.getColumnIndexOrThrow(MediaColumns.DATE_MODIFIED);
		_colonneDateTaken = _cursor.getColumnIndexOrThrow(VideoColumns.DATE_TAKEN);
		_colonneDateAdded = _cursor.getColumnIndexOrThrow(MediaColumns.DATE_ADDED);
	}

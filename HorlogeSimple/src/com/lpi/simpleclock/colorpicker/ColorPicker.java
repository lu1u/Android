...
/*		if (mShowCenterOldColor)
		{
			...
			canvas.drawArc(mCenterRectangle, -45, 90, true, mCenterOldPaint);

			// Draw the new selected color in the center.
			canvas.drawArc(mCenterRectangle, 45, 270, true, mCenterNewPaint);
		} else*/
		{
			// Draw the new selected color in the center.
	...
	/**
	 * Set the color to be highlighted by the pointer. </br> </br> If the instances {@code SVBar}
	 * and the {@code OpacityBar} aren't null the color will also be set to them
	 * 
	 * @param color
	 ...
	 *            resulting color often won't look close to the original color. This is especially

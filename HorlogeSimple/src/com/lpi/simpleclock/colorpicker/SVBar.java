...
public class SVBar extends View
{
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh)
	{
		...
		if (!isInEditMode())
		{
			...
			{ 0xffffffff, Color.HSVToColor(mHSVColor), 0xff000000 }, null, Shader.TileMode.CLAMP);
		} else
		{
			shader = new LinearGradient(mBarPointerHaloRadius, 0, x1, y1, new int[]
			{ 0xffffffff, 0xff81ff00, 0xff000000 }, null, Shader.TileMode.CLAMP);
		...
		{
			mBarPointerPosition = Math.round((mSVToPosFactor * (1 - hsvColor[2])) + mBarPointerHaloRadius
					+ (mBarLength / 2));
		}
		if (isInEditMode())
		{
	...
	public void setColor(int color)
	{
		...
		shader = new LinearGradient(mBarPointerHaloRadius, 0, x1, y1, new int[]
		{ 0xffffffff, color, 0xff000000 }, null, Shader.TileMode.CLAMP);
		mBarPaint.setShader(shader);
		calculateColor(mBarPointerPosition);
		mBarPointerPaint.setColor(mColor);
		if (mPicker != null)
	...
	private void calculateColor(int coord)
	{
		...
		} else if (coord > mBarPointerHaloRadius && coord < (mBarPointerHaloRadius + mBarLength))
		{
			...
			{ mHSVColor[0], ((mPosToSVFactor * (coord - mBarPointerHaloRadius))), 1f });

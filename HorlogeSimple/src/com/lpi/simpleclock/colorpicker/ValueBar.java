package com.lpi.simpleclock.colorpicker;
...
public class ValueBar extends View
{
	 ...
	 */
	private static final String STATE_PARENT = "parent"; //$NON-NLS-1$
	private static final String STATE_COLOR = "color"; //$NON-NLS-1$
	private static final String STATE_VALUE = "value"; //$NON-NLS-1$
	...
	/**
	 * Interface and listener so that changes in ValueBar are sent to the host activity/fragment
	 */
	...
	private void init(AttributeSet attrs, int defStyle)
	{
		...
		mBarPointerRadius = a.getDimensionPixelSize(R.styleable.ColorBars_bar_pointer_radius,
				b.getDimensionPixelSize(R.dimen.bar_pointer_radius));
		mBarPointerHaloRadius = a.getDimensionPixelSize(R.styleable.ColorBars_bar_pointer_halo_radius,
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh)
	{
		...
		if (mOrientation == ORIENTATION_HORIZONTAL)
		{
			...
			mBarRect.set(mBarPointerHaloRadius, (mBarPointerHaloRadius - (mBarThickness / 2)),
					(mBarLength + (mBarPointerHaloRadius)), (mBarPointerHaloRadius + (mBarThickness / 2)));
		} else
		{
			x1 = mBarThickness;
			y1 = (mBarLength + mBarPointerHaloRadius);
			mBarLength = h - (mBarPointerHaloRadius * 2);
			mBarRect.set((mBarPointerHaloRadius - (mBarThickness / 2)), mBarPointerHaloRadius,
					(mBarPointerHaloRadius + (mBarThickness / 2)), (mBarLength + (mBarPointerHaloRadius)));
		}
		...
		if (!isInEditMode())
		{
			...
			{ Color.HSVToColor(0xFF, mHSVColor), Color.BLACK }, null, Shader.TileMode.CLAMP);
		} else
		{
			shader = new LinearGradient(mBarPointerHaloRadius, 0, x1, y1, new int[]
			{ 0xff81ff00, Color.BLACK }, null, Shader.TileMode.CLAMP);
